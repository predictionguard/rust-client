...
mod tests {
    ...
    fn chat_completion() {
        ...
        let chat_completion_mock = server.mock(|when, then| {
            when.method(POST).path(completion::CHAT_PATH);
            then.status(200)
                .header("Content-Type", "application/json")
        ...
        tokio_test::block_on(async {
            ...
            assert!(choices[0].index >= 0);
            assert_eq!(choices[0].message.role, completion::Roles::Assistant);
    ...
    fn embedding() {
        ...
        let inputs = vec![embedding::Input {
            text: "Skyline with Airplane".to_string(),
    ...
    const EMBEDDING_RESPONSE: &str = r#"{ "id": "emb-DMC7M45FkuwJ9ihyP23RKrC6hUXwg", "object": "embedding_batch", "created": 1717015553, "model": "BridgetowerLargeItmMlmItc", "data": [{"status": "success","index": 0,"object": "embedding",
          "embedding": [
